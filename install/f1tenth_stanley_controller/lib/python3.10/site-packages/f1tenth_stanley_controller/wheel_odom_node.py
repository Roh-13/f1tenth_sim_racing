...
class WheelOdomNode(Node):
    ...
    def update_odometry(self):
        ...
        self.theta= self.theta % (2 * math.pi) - math.pi * (self.theta > math.pi)
        ...
        transform.header.stamp = odom_msg.header.stamp = now.to_msg()
        transform.header.frame_id = odom_msg.header.frame_id = 'odom'
        transform.child_frame_id = odom_msg.child_frame_id = 'f1tenth_1'

                   
        
        transform.transform.translation.x = odom_msg.pose.pose.position.x = self.x
        transform.transform.translation.y = odom_msg.pose.pose.position.y = self.y
        transform.transform.translation.z = 0.0
        # Orientation (convert to quaternion)
        q = tf_transformations.quaternion_from_euler(0, 0, self.theta)
        transform.transform.rotation = odom_msg.pose.pose.orientation = Quaternion(x=q[0], y=q[1], z=q[2], w=q[3])
